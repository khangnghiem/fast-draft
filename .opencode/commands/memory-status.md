---
description: Show read-only agent-memory status
agent: build
---
...
1. Run `agentmem repo read-config` from the project root and report `project_id`, `scratch_dir`, resolved `projectSubtree`, and canonical docs summary.
...
Do not run `git pull`, `git push`, `git add`, or `git commit` from this command unless the user explicitly switches to `/memory-sync`.
