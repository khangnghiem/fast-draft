...
pub struct FdCanvas {
    ...
    /// group, we keep the group selected for dragging and store the child here.
    /// On pointer-up without drag, we drill into this child.
...
impl FdCanvas {
    ...
    pub fn handle_pointer_down(
        ...
        let hit = raw_hit.map(|id| {
            ...
            // If drill-down resolved to a child of an already-selected group,
            // keep the group as hit target so dragging moves the whole group.
            // Store the child as a pending drill target for click-without-drag.
