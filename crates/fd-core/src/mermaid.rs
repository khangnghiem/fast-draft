...
fn parse_flowchart(input: &str) -> Result<SceneGraph, String> {
    ...
    let mut current_subgraph_seen_ids: HashSet<String> = HashSet::new();
    ...
    for line in lines {
        ...
        if trimmed.starts_with("subgraph") {
            ...
            current_subgraph_seen_ids.clear();
            continue;
        }
        ...
        if let Some(parsed_edges) = try_parse_edge_line(trimmed) {
            ...
            if let Some(ref mut sg) = current_subgraph {
                for pe in &parsed_edges {
                    if !current_subgraph_seen_ids.contains(&pe.from) {
                        current_subgraph_seen_ids.insert(pe.from.clone());
                        sg.node_ids.push(pe.from.clone());
                    }
                    if !current_subgraph_seen_ids.contains(&pe.to) {
                        current_subgraph_seen_ids.insert(pe.to.clone());
        ...
        if let Some(node) = try_parse_node_def(trimmed) {
            if let Some(ref mut sg) = current_subgraph
                && !current_subgraph_seen_ids.contains(&node.id)
            {
                current_subgraph_seen_ids.insert(node.id.clone());
