...
3. **Commit known durable memory outputs only**
   - If every dirty path is a recognized durable memory output from `agentmem promote` or normal memory edits (`projects/**`, `lessons/**`, `snippets/**`, `web/**`, `inbox/**`, `drafts/**`, `preferences.md`, `README.md`), scan the exact candidate paths with `bash ~/.config/memory/scripts/check-secrets.sh <paths>`.
