...
## 2024-05-24 - Dynamic CORS configuration for Webviews and Localhost
**Vulnerability:** Overly permissive `Access-Control-Allow-Origin: '*'` allowing any site to access the AI API endpoint, leading to potential abuse and rate-limiting issues for legitimate users.
**Learning:** Cloudflare Pages Functions require dynamic generation of CORS headers because they must support both standard web protocols (`https://fast-draft.com`) and custom protocols like `vscode-webview://` and `tauri://`. Using a static `*` is a security risk but hardcoding a single string breaks multi-environment support.
**Prevention:** Instead of `*`, always validate the `Origin` header dynamically against a predefined Set of allowed domains and prefix strings (for custom protocols). Ensure `Vary: Origin` is always included so the browser caches correctly per origin, and conditionally append `Access-Control-Allow-Origin` only if there's a match.
