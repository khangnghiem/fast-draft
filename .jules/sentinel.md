...
## 2026-03-10 - Insufficient HTML Escaping Logic Leads to XSS Risk
**Vulnerability:** Multiple `escapeHtml` implementations across the codebase (`site/app.js`, `site/search.js`, `site/ai-chat.js`, `fd-vscode/webview/src/ai-chat.js`, and `fd-vscode/src/fd-parse.ts`) failed to escape single quotes (`'`), and one implementation relied insecurely on `innerHTML`.
**Learning:** Inconsistent escaping functions can leave edge-case vulnerabilities like attribute-based XSS when single quotes are used. Additionally, relying on DOM operations (`innerHTML`) for string escaping is unsafe and error-prone.
**Prevention:** Standardize a single, robust `escapeHtml` utility using defensive type coercion (`String()`) and comprehensive regex replacements for `&`, `<`, `>`, `"`, and `'`.
