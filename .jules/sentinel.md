...
## 2024-05-18 - Missing DOMPurify Sanitization on Rendered Markdown Notes
**Vulnerability:** The Fast Draft website (`site/app.js`) rendered user-provided markdown notes into HTML using `marked.parse(processedNote)` and directly appended the result to the `innerHTML` of the specs panel, allowing for DOM-based XSS if the notes contained malicious HTML.
**Learning:** Even when markdown parsing itself provides some safety, adding it to a broader `innerHTML` construction requires explicit sanitization of the rendered markdown output. We must sanitize *only* the specific untrusted input (the rendered markdown) instead of the entire assembled HTML string, to prevent stripping valid UI structural elements that we construct dynamically in the template string.
**Prevention:** Always apply `window.DOMPurify.sanitize()` (with the necessary `ADD_ATTR` options for safe custom data attributes) to the rendered markdown variable before interpolating it into the larger HTML template string. Use conditional sanitization to avoid crashes when DOMPurify is unavailable.
