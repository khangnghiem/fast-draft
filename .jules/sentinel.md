...
## 2026-03-18 - DOM XSS in Animation Picker
**Vulnerability:** Untrusted node animation trigger names from the extension's `.fd` files were directly interpolated into `row.innerHTML` in `fd-vscode/webview/src/drag-drop.js` and `fd-vscode/webview/main.js`.
**Learning:** Even internal toolings or simple UI renders built around native data files can be susceptible to XSS if inputs can be controlled (e.g., untrusted files). Using `innerHTML` with untrusted data allows execution of arbitrary scripts.
**Prevention:** Avoid `innerHTML` for dynamically generating DOM nodes. Always use safer DOM APIs like `document.createElement()` and `textContent` when rendering untrusted or user-controlled data. In this case, `row.firstElementChild.textContent` was used to safely insert the text content after the static HTML structure was rendered.
