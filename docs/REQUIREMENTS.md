...
- **R3.6** _(done)_: Pan (Space+drag, middle-click, ⌘-hold, Hand tool on empty space), zoom (see R3.20), grid (see R3.21); Hand tool is context-aware: pan on empty space, select+move on nodes; two-finger gestures always pan regardless of tool/target
