...
- **R6.18** _(done)_: UI refinements v2 — Apple-style 28px icon buttons replace frosted pill chrome (no canvas overlap); 3 tabs per panel with segmented controls (left: Code/Inspect/Export, right: Agent/Settings/History); settings panel with grouped items replaces dropdown; left panel default 320px user-resizable; Excalidraw-style onboarding overlay (first-visit tips, dismiss on click/key, localStorage tracked); responsive minimap (viewport-proportional sizing, toolbar collision avoidance)
