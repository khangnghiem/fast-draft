...
- **R6.7** _(done)_: Resizable panels — layers and properties panels are drag-to-resize with accent-highlighted handles; double-click handle to collapse (0px); click restore strip to uncollapse; widths persist via `localStorage` (site) / `vscode.setState()` (extension); canvas area dynamically adjusts via CSS variables `--layers-width` / `--props-width`; floating toolbar tracks layers width; at ≤768px viewport, `--layers-width` and `--props-width` reset to `0px` so canvas fills full width; layers panel accessible as slide-in drawer with toggle button and backdrop overlay
