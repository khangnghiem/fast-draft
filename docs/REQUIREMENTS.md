...
- **R3.39** _(done)_: Floating toolbar — draggable toolbar with 7 tool buttons (SVG icons); drag handle with grip; snaps to 4 edges within canvas bounds (never behind panels); preserves orientation during drag; free-position snap (anywhere along edge, clamped within canvas); cursor-following snap shadow shows destination at pointer position; free-float on canvas interior (no forced edge dock); auto-dock on overflow; **canvas containment invariant: toolbar must always stay within visible canvas area, re-clamped on panel toggle, window resize, or any layout change**; velocity throw; double-click grip to minimize; state persists via localStorage; re-clamps on window resize and panel toggle → [spec](specs/floating-toolbar.md)
