...
- **R3.2** _(done)_: Drag, resize, rotate; Shift-constrain to axis; nested child constraints maintain visual coordinate stability symmetrically using inverse dimension mapping → [spec](specs/selection.md)
...
- **R3.38** _(replaced)_: ~~Context-menu reparent on drop~~ — replaced by ⌘+drag nest+center gesture (see R3.64)
- **R3.39** _(done)_: Floating toolbar — draggable toolbar with 7 tool buttons (SVG icons); drag handle with grip; snaps to 4 edges within canvas bounds; **toolbar-rect-based snap detection** (projects toolbar bounding rect from cursor + grab offset, snaps when any toolbar edge is within 60px of canvas edge — closest edge wins); **grab-offset preservation** (shadow and snap position respect where user grabbed the grip, not cursor center); free-position snap (anywhere along edge, clamped within canvas); snap shadow shows exact landing position aligned with drag position **and swaps dimensions to match target edge orientation**; free-float on canvas interior (no forced edge dock); auto-minimize on panel overlap; **canvas containment invariant: toolbar safely ignores overlay panels but dynamically minimizes if physically touched by an opening panel**; velocity throw; **grip-anchored minimize** (double-click grip to minimize/expand; grip stays stationary, toolbar shrinks/expands toward it); auto-minimization achieves true zero-gap; state persists via localStorage; re-clamps on window resize → [spec](specs/floating-toolbar.md)
...
- **R3.59** _(done)_: Clipboard — ⌘C copies selected node(s) (multi-select supported); ⌘V pastes with +20px cumulative offset (not stacked on top); ⌘X cuts (copy + delete); paste is undoable via `push_undo_snapshot` WASM API; intelligently extracts code within markdown fences and provides fallback UI toasts
...
- **R3.84** _(done)_: Center-snap with dashed highlight — dragging a text node within 20px of a shape/edge center shows a dashed orange `#FF9500` highlight with crosshair; on release, applies `CenterIn` constraint via `center_node_in()` without reparenting (Toast notification now includes ⌘+Drag nesting tip); new `get_center_snap()` WASM API returns JSON snap target info, excluding false-positives during text resize
- **R3.85** _(done)_: Text-priority hit-test — text nodes always win over overlapping shape siblings in hit-testing regardless of document order; two-pass approach in `hit_test_node()` and `hit_test_node_excluding()` checks text children first, then non-text; `SpatialIndex` gives text nodes a z-order boost (+10,000) for consistent fast-path results
...
- **R3.64** _(done)_: ⌘+drag reparent — holding ⌘/Ctrl while dragging a node moves with all children; dropping onto a different container (Rect/Ellipse/Frame/Group, not Text) auto-nests and centers via `reparent_into_centered()` WASM API; releasing on empty space or same parent is standard move-with-children
...
- **R3.69** _(done)_: Layers panel file explorer UX — ⌘+Click multi-select (toggle), ⇧+Click batch-select (range), keyboard shortcuts (Delete/⌘C/⌘X/⌘V/⌘D/⌘A) when panel has focus, context menu with Duplicate/Copy/Paste/Delete actions, persistent hover-activated Trash Bin button; position-preserving reparent (fixes text node disappearance and identity duplication); WASM `toggle_select_by_id()`, `add_to_selection()`, `select_multiple_by_ids()` APIs
...
- **R3.78** _(done)_: Context Menu Interactions — unified, keyboard-navigable context menus across canvas and layers panel for nodes, edges, and document-level actions; smart right-click gesture with drag-to-pan fallback → [spec](specs/context-menu.md)

#### R3c: Navigation & View

- **R3.6** _(done)_: Pan (Space+drag, middle-click, right-click, ⌘-hold, Hand tool), zoom (see R3.20), grid (see R3.21); Hand tool is pan-only (click+drag to pan, no selection or node dragging); two-finger gestures always pan regardless of tool/target; normalized zoom wheel factor (`ZOOM_WHEEL_FACTOR = 1.04`) shared across platforms; smart two-finger disambiguation (50ms delay, 30px min distance, Apple Pencil palm rejection); touch inertia with weighted 3-frame velocity average and 0.95 friction; three-finger swipe undo/redo; long-press context menu (500ms, Select/Eraser tool only); iPadOS-style gesture hierarchy: 3-finger tap=undo, double-tap=redo, pinch-in=copy, pinch-out=paste, long-press=edit menu; 4-finger tap=zen toggle, swipe up=zoom-to-fit, swipe down=zoom-to-selection, horizontal swipe=tool cycle; Select tool is default on app launch.
...
- **R3.27** _(done)_: Layer rename — double-click layer name for inline rename; string mutations implement an explicit strict ID collision check (`has_node()`) to prevent logical desynchronization; renames `@id` document-wide → [spec](specs/inline-editing.md)
...
- **R6.18** _(done)_: UI refinements v2 — Apple-style 28px icon buttons (offset by panel widths, never overlap panels); Layers merged into left panel tabs (Layers/Code/Inspect); right panel tabs Agent/Export/Settings; bottom-anchored panel headers to assert UX structural symmetry across both `Layers` and `Code` fields; handwritten Caveat-font onboarding hints on canvas only (no overlay, auto-dismiss 8s, pointer-events none); Agent header deduped; left panel 320px user-resizable; responsive minimap (viewport-proportional, toolbar collision avoidance)
...
- **R6.21** _(done)_: Adaptive sidebar toggle icons — sidebar toggle buttons appear in panel tab headers when panels are open (and hide from chrome using an opacity fade transition); when panels collapse, chrome toggle icons fade back in; panel z-index raised to 25 (above chrome at 20) for proper overlay
...
| R3.4        | `tool_pen_basic_draw`, `tool_pen_two_points`, `tool_pen_cancel`, `tool_pen_subsampling`, `tool_pen_catmull_rom_smoothing`, `tool_pen_pressure_data_capture`                                                                                                                                                                          | ✅ 6 unit tests |
| R3.5        | _(planned)_                                                                                                                                                                                                                                                        | —                              |
...
| R3.16       | `hit_test_resize_handle` (WASM), E2E UX cursor tests                                                                                                                                                                                                               | ✅ 3 tests                     |
...
| R4.7–R4.11  | `exportSpec.test.ts`, `ai-touch.test.ts`                                                                                                                                                                                                                           | ✅                             |
...
| R3.46       | `test_update_text_metrics_updates_bounds_and_spatial_index`                                                                                                                                                                                                        | ✅ 1 WASM test                 |
